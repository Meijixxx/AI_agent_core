...
def format_japanese_markdown(input_path: str, output_path: str) -> str:
    """日本語技術/規則文書の生テキストに、パターンベースで見出しマークアップを
    機械的に追加する（LLM経由なし、内容の改変ゼロ）。

    検出パターン:
      - 「第N編」    → # 第N編
      - 「第N章」    → ## 第N章
      - 「第N条」    → ### 第N条（括弧タイトルがあれば結合）
      - 「第N節」    → #### 第N節
      - 「第N項」    → ##### 第N項
      - 「別表N」「別紙N」「付録N」 → ## として扱う
    本文は一切変更しない。PDF抽出時の余分な空白や連続改行は整える。
    """
    ...
    patterns: list[tuple[re.Pattern, str]] = [
        (re.compile(r"^\s*第\s*[０-９0-9]+\s*編(?:\s*[　 ]*.*)?\s*$"), "# "),
        (re.compile(r"^\s*第\s*[０-９0-9]+\s*章(?:\s*[　 ]*.*)?\s*$"), "## "),
        (re.compile(r"^\s*第\s*[０-９0-9]+\s*条(?:\s*（[^）]*）)?\s*$"), "### "),
        (re.compile(r"^\s*第\s*[０-９0-9]+\s*節(?:\s*[　 ]*.*)?\s*$"), "#### "),
        (re.compile(r"^\s*第\s*[０-９0-9]+\s*項(?:\s*[　 ]*.*)?\s*$"), "##### "),
        (re.compile(r"^\s*(?:別表|別紙|付録|附則)\s*[０-９0-9]*(?:\s*[　 ]*.*)?\s*$"), "## "),
    ]

    lines = text.splitlines()
    out_lines: list[str] = []
    heading_count = 0
    prev_blank = True  # ファイル先頭は空行扱い

    for line in lines:
        # 行内の多重スペースを1つに（行頭インデントは保持）
    ...
    return f"整形完了: {abs_out} ({heading_count}個の見出しを検出, {size:,} bytes)"
